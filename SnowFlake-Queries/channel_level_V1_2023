...
, trn_flags AS (
    ...
    FROM (
        ...
        WHERE 
            o.ORDERDATE BETWEEN   '2023-01-29'  and '2024-02-03'
...
select brand,sku,
CASE
    WHEN brand IN ('BHCA','BHDE','BHFR','BHUK','BHUS','BHAU') THEN 'Ecom'
    WHEN brand = 'BHS' THEN 'Store'
    ELSE 'Indirect'
END AS channel,
sum(QUANTITY_RECEIVED) as receipt_units from
(
SELECT distinct
  po.sku AS sku, 
...
FROM GDGIHQE_BH_PROD_IA_PROD_SHARE.IMPACT_ANALYTICS.vw_purchase_order_most_recent_upload AS po
 ...
 JOIN (
  ...
  AND pl.container_number = wr.container_number
    ...
    where cast(wr.date_received as date) between   '2023-01-29'  and '2024-02-03'
    )
    group by brand,sku
    ),
...
finall_pre AS (
    SELECT
        ...
        CASE WHEN SUM(t.UNITSOLD * t.base_price) = 0 THEN NULL
             ELSE ((SUM(t.UNITSOLD * t.base_price) - (SUM(t.UNITSOLD * COALESCE(t.PRODUCTPRICE,0)) - SUM(COALESCE(t.discount_amount,0))))
    ...
    LEFT JOIN pri p
        ON t.BRAND = p.BRAND AND t.SKU = p.SKU AND t.orderdate = p.date AND t.currency = p.currency
...
finall_with_flags AS (
    SELECT
        f.*,
        sum(COALESCE(tf.rtp_units, 0)) AS rtp_units,
        sum(COALESCE(tf.rtp_revenue, 0)) AS rtp_revenue,
        sum(COALESCE(tf.pr_units, 0)) AS pr_units,
        sum(COALESCE(tf.pr_revenue, 0)) AS pr_revenue,
        sum(COALESCE(tf.warranty_units, 0)) AS warranty_units,
        sum(COALESCE(tf.warranty_revenue, 0)) AS warranty_revenue,
        sum(COALESCE(po.receipt_units, 0)) AS receipt_units,
        (f.revenue / NULLIF(f.units,0)) * COALESCE(po.receipt_units,0) AS receipt_revenue
    FROM finall_pre f
